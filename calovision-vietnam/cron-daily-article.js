...
export default async function handler(req, res) {
  ...
  const maxPerRun = config.maxArticlesPerDay || 5;
  const githubToken = process.env.GITHUB_TOKEN || req.query.token;
  const apiKey = process.env.GEMINI_API_KEY;

  // Find up to maxPerRun pending topics
  const pendingTopics = queue.filter(item => item.status === 'pending').slice(0, maxPerRun);
  if (pendingTopics.length === 0) {
    return res.status(200).json({
      success: true,
      message: 'Tất cả 180 chủ đề trong Content Queue đã được xuất bản!'
    });
  }
  ...
  try {
    ...
    // Save updated queue back to GitHub
    if (githubToken) {
      await commitFileToGitHub('content-queue.json', JSON.stringify(data, null, 2), `Update queue status for ${generatedResults.length} articles`, githubToken);
    }
    ...
    return res.status(200).json({
      ...
      message: `Đã xuất bản tự động ${generatedResults.length} bài viết mới thành công!`,
      publishedArticles: generatedResults
    });

  } catch (err) {
    console.error('Error in daily cron batch:', err);
...
function generateFullFeaturedArticleHtml(topicObj, slug, publishedArticles) {
  const title = `${topicObj.topic} Bao Nhiêu Calo? [Chi Tiết Kcal] – CaloVision`;
  ...
  <div class="main-layout-container" style="max-width: 900px;">
    <article class="seo-content-section" style="margin-top: 1.5rem;">
      ...
      <div style="border-radius: var(--radius-md); overflow: hidden; margin-bottom: 1.5rem;">
        <img src="https://images.unsplash.com/photo-1546069901-ba9599a7e63c?auto=format&fit=crop&w=1000&q=80" alt="Suất ${foodName} thơm ngon cung cấp calo chuẩn y khoa" style="width: 100%; height: 360px; object-fit: cover;">
      </div>
